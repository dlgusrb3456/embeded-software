...
static int __init simple_char_init(void){
	...
	alloc_chrdev_region(&dev_num,0,1,DEV_NAME);
       	//to get major number. choose the number, which is not use and put the number in  dev_num
	cd_cdev = cdev_alloc();
	cdev_init(cd_cdev, &simple_char_fops);
	cdev_add(cd_cdev,dev_num,1);
	//register device driver to kernel
	printk("simple_char: init module end \n");	
	return 0;
}
...
static void __exit simple_char_exit(void){
	...
	printk("simple_char: exit module end \n");
	//release register character device
}
